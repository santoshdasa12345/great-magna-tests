...
SELECTORS = {
    "dashboard": {
        "learn to export": Selector(
            By.XPATH,
            "//a[@id='header-link-learning']",  # "//a[@id='header-link-learning']"
        ),
        ...
        "build an export plan": Selector(
            By.XPATH,
            "//a[@id='header-link-exporting-plan']",  # "//a[@id='header-link-exporting-plan']"
        ),
        "sign out": Selector(By.XPATH, "//span[contains(text(),'Sign out')]"),
        ...
        "i have exported in last 12 months": Selector(
            By.XPATH,
            "//label[contains(text(),'I have exported in the last 12 months')]",
        ),
        "i have exported before but not in the last 12 months": Selector(
            By.XPATH,
            "//label[contains(text(),'I have exported before but not in the last 12 mont')]",
        ),
        "i have never exported but have a product or service": Selector(
            By.XPATH,
            "//label[contains(text(),'I have never exported but have a product or servic')]",
        ),
        "i do not have a product or service for export": Selector(
            By.XPATH,
            "//label[contains(text(),'I do not have a product or service for export')]",
        ),
        "save": Selector(By.XPATH, "//button[contains(text(),'Save')]"),
        "go to export plan": Selector(
            By.XPATH, "//a[contains(text(),'Go to export plan')]"
        ),
        "start": Selector(By.XPATH, "//a[contains(text(),'Start')]"),
        "export plan progress bar": Selector(
            By.XPATH,
            "//body/main[@id='content']/div[1]/div[1]/div[1]/div[1]/div[1]/div[2]/div[1]/div[1]",
        ),
        ...
        "search next button": Selector(
            By.XPATH,
            "//body/div[4]/div/div/form/div[2]/div/span/div/section[1]/div/fieldset/button"
        ...
        "home": Selector(By.XPATH, "//span[contains(text(),'Home')]"),
        ...
        "menu advice": Selector(By.XPATH, "//span[contains(text(),'Advice')]"),
        "menu markets": Selector(By.XPATH, "//span[contains(text(),'Markets')]"),
        ...
        "continue": Selector(By.XPATH, "//a[contains(text(),'Continue')]"),
        "coming soon": Selector(By.XPATH, "//button[contains(text(),'Coming soon')]"),
        "privacy and cookies": Selector(
            By.XPATH,
            "//a[contains(text(),'Privacy and cookies')]",
            type=ElementType.LINK,
        ),
        ...
        "footer logo": Selector(By.CSS_SELECTOR, "#footer > nav > div > a > img"),
        ...
        "my markets": Selector(
            By.CSS_SELECTOR,
            "#set-country-button > span > span:nth-child(1) > button > span",
        ),
        "add market": Selector(
            By.CSS_SELECTOR,
            "#set-country-button > span > span:nth-child(1) > div > button",
        ),
        "vfm close": Selector(By.XPATH, "//body/div[7]/div/div/button"),
        "not now": Selector(By.XPATH, "//body/div[7]/div/div/div/div[2]/button[1]"),
...
def fill_out_product(driver: WebDriver, product_name: str):
    ...
    product_btn.click()
    add_product_btn = find_element(
        driver, find_selector_by_name(SELECTORS, "add product")
    )
    add_product_btn.click()
    ...
    time.sleep(2)
    driver.find_element_by_xpath(
        "//body/div[3]/div/div/form/div[2]/div/div/div[1]/label/div/input"
    ).clear()
    driver.find_element_by_xpath(
        "//body/div[3]/div/div/form/div[2]/div/div/div[1]/label/div/input"
    ).send_keys(product_name)
    driver.find_element_by_xpath(
        "//body/div[3]/div/div/form/div[2]/div/div/div[1]/button"
    ).click()
...
def fill_out_country(driver: WebDriver, country: str):
    ...
    country_btn = find_element(
        driver,
        find_selector_by_name(SELECTORS, "country-btn"),  # dashboard add country button
    )
    country_btn.click()
...
def search_select_save_radio(driver: WebDriver):
    ...
    while True:
        ...
        # look for div's and radio buttons
        parent_1_div_element = driver.find_element_by_xpath(
            "//body/div[4]/div/div/form/div[2]/div/span/div/section[1]/div"
        )  # ("interaction grid m-v-xs")
        # child_1_div_element = parent_1_div_element.find_element_by_tag_name("div")  # ("c-fullwidth")
        main_div_element = parent_1_div_element.find_element_by_tag_name(
            "div"
        )  # "m-b-xs"
        # radio button labels
        label_elements = main_div_element.find_elements_by_tag_name("label")
...
def find_and_select_random_export_plan(driver: WebDriver):
    div_list_export_plan_xpath = driver.find_element_by_xpath(
        "//body/main/div/div/div[1]/div"
    )
    export_plan_xpath_div_1_elements = (
        div_list_export_plan_xpath.find_elements_by_tag_name("div[1]")
    )
    random_number = 1
    ...
    export_plan_xpath_div_2_elements = (
        export_plan_xpath_div_1_elements.find_element_by_tag_name("div")
    )
